...
def test_signup_returns_tokens():
    async def _go():
        f = await _db()
        async with f() as db:
            r = await service.signup(db, _SIGNUP)
            await db.commit()
            return r
...
def test_signup_duplicate_email_raises_400():
    async def _go():
        f = await _db()
        async with f() as db:
            await service.signup(db, _SIGNUP)
            await db.commit()
...
def test_login_returns_tokens():
    async def _go():
        ...
        async with f() as db:
            ...
            await db.commit()
        async with f() as db:
            r = await service.login(db, LoginRequest(email=_SIGNUP.email, password=_SIGNUP.password))
            await db.commit()
            return r
...
def test_refresh_returns_new_tokens():
    async def _go():
        f = await _db()
        async with f() as db:
            tokens = await service.signup(db, _SIGNUP)
            await db.commit()
        async with f() as db:
...
def test_logout_revokes_refresh_token():
    async def _go():
        ...
        async with f() as db:
            tokens = await service.signup(db, _SIGNUP)
            await db.commit()
        # logout — get user_id from token payload
...
def test_update_me_weekly_intentions_none_leaves_unchanged():
    async def _go():
        ...
        async with f() as db:
            ...
            await db.commit()
        # Then patch only mascot_name — intentions should be untouched
        async with f() as db:
            user = await service.get_user_by_id(db, user_id_str)
